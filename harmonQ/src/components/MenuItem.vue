<template>
  ...
  >
    ...
    >
      <div class="left" ref="labelRef" :class="{ marginAuto: miniMenu && depth === 0, collapseEnd: miniMenu }">
        ...
        <template v-if="labelName">
          ...
          <component v-else :labelName="labelName" :active="active" :miniActive="miniActive" :isChildrenMenuOpen="showChildren" :is="menuItemLabel" />
        </template>
      </div>
      <template v-if="(miniMenu && depth != 0) || !miniMenu">
        <div v-if="item.children && !itemApendIcon" class="icons postIconOpenAnima" :class="{ opened: showChildren }"></div>
  ...
  computed: {
    ...
    menuItemClass() {
      ...
      return {
        ...
        menuExpanded:
          this.menuType === 'fully' && ((!this.miniMenu && this.expanded && this.depth == 0) || (this.miniMenu && this.depth == 1 && this.expanded)),
